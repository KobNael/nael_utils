...
{
    EXPECT_EQ(compute_lcm({7}), 7UL);
}

TEST(lcm, vector_multiple_values)
{
    EXPECT_EQ(compute_lcm({4, 6}), 12UL);
    EXPECT_EQ(compute_lcm({2, 3, 4}), 12UL);
    EXPECT_EQ(compute_lcm({1, 1, 1}), 1UL);
}

TEST(lcm, vector_throws_on_empty)
{
    EXPECT_THROW(compute_lcm({}), fatal_error);
}
